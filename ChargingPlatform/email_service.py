...
async def send_charging_invoice(
    ...
    # Cost-breakdown rows (compact)
    if has_deposit_flow:
        idle_row = ""
        if idle_minutes and idle_minutes > 0 and idle_fee:
            idle_row = (
                '<tr><td style="padding:3px 0;color:#64748B;font-weight:600;">Idle fee</td>'
                f'<td style="text-align:right;color:#E67E22;font-weight:700;">− RM {idle_fee:.2f} '
                f'<span style="color:#94A3B8;font-weight:400;font-size:10px;">({idle_minutes} min)</span></td></tr>'
            )
        cost_rows = f"""
            <tr><td style="padding:3px 0;color:#64748B;font-weight:600;width:55%;">Deposit captured</td><td style="text-align:right;color:#0F172A;font-weight:700;">RM {hold_amount:.2f}</td></tr>
            <tr><td style="padding:3px 0;color:#64748B;font-weight:600;">Energy used</td><td style="text-align:right;color:#0F172A;font-weight:700;">− RM {(energy_cost or 0):.2f}</td></tr>
            {idle_row}
            <tr><td colspan="2" style="border-top:2px solid #00C266;padding-top:6px;"></td></tr>
            <tr><td style="padding:5px 0;color:#0F172A;font-size:13px;font-weight:700;">Refunded to TNG</td>
                <td style="text-align:right;color:#00A852;font-size:17px;font-weight:800;">RM {refund_amount:.2f}</td></tr>
        """
    else:
        cost_rows = (
            '<tr><td colspan="2" style="border-top:2px solid #00C266;padding-top:6px;"></td></tr>'
            f'<tr><td style="padding:5px 0;color:#0F172A;font-size:13px;font-weight:700;width:55%;">Amount paid</td>'
            f'<td style="text-align:right;color:#00A852;font-size:17px;font-weight:800;">RM {amount_paid:.2f}</td></tr>'
        )
  ...
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr><td align="center">
      ...
      <table class="card" width="600" cellpadding="0" cellspacing="0" style="background:#FFFFFF;border-radius:16px;overflow:hidden;box-shadow:0 4px 18px rgba(15,23,42,0.06);max-width:600px;width:100%;">
        ...
        <!-- Hero band -->
        <tr><td class="hero" style="background:linear-gradient(135deg,#0A2A18 0%,#00A852 100%);padding:16px 20px 14px;text-align:center;">
          <img class="hero-logo" src="https://charger.czeros.tech/static/logo.png" alt="PlagSini" width="68" style="display:block;margin:0 auto 6px;border:0;outline:none;text-decoration:none;-ms-interpolation-mode:bicubic;">
          <div class="hero-name" style="color:#FFFFFF;font-size:17px;font-weight:900;letter-spacing:-0.3px;line-height:1;">Plag<span style="color:#A8FFD3;">Sini</span></div>
          <div class="hero-tag" style="color:rgba(255,255,255,0.7);font-size:8px;font-weight:700;letter-spacing:2.2px;text-transform:uppercase;margin-top:5px;">Charge Your Journey</div>
        </td></tr>

        <!-- Receipt title + kWh hero -->
        <tr><td class="title-pad" style="padding:14px 24px 0;text-align:center;">
          <div style="font-size:9px;color:#94A3B8;letter-spacing:1.6px;text-transform:uppercase;font-weight:700;">Charging Receipt</div>
          <div class="kwh-hero" style="font-size:28px;color:#0F172A;font-weight:900;margin:4px 0 2px;letter-spacing:-1px;line-height:1;">{energy_kwh:.2f} <span style="font-size:14px;font-weight:600;color:#64748B;">kWh</span></div>
          <div style="font-size:9px;color:#64748B;font-family:'SF Mono',Consolas,monospace;">{transaction_ref}</div>
        </td></tr>

        <!-- Stat strip: 3 columns -->
        <tr><td class="stat-pad" style="padding:10px 24px 2px;">
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#F8FAFC;border-radius:8px;border:1px solid #E2E8F0;">
            <tr>
              <td class="stat-cell" style="padding:9px 4px;text-align:center;border-right:1px solid #E2E8F0;width:33%;">
                <div style="font-size:8px;color:#64748B;text-transform:uppercase;letter-spacing:0.9px;font-weight:700;margin-bottom:3px;">Duration</div>
                <div class="stat-val" style="font-size:13px;color:#0F172A;font-weight:800;font-family:'SF Mono',Consolas,monospace;">{duration_str}</div>
              </td>
              <td class="stat-cell" style="padding:9px 4px;text-align:center;border-right:1px solid #E2E8F0;width:33%;">
                <div style="font-size:8px;color:#64748B;text-transform:uppercase;letter-spacing:0.9px;font-weight:700;margin-bottom:3px;">Avg Power</div>
                <div class="stat-val" style="font-size:13px;color:#0F172A;font-weight:800;">{avg_kw_str}</div>
              </td>
              <td class="stat-cell" style="padding:9px 4px;text-align:center;width:33%;">
                <div style="font-size:8px;color:#64748B;text-transform:uppercase;letter-spacing:0.9px;font-weight:700;margin-bottom:3px;">CO₂ saved</div>
                <div class="stat-val" style="font-size:13px;color:#00A852;font-weight:800;">{co2_kg} kg</div>
              </td>
            </tr>
          </table>
        </td></tr>

        <!-- Session details (plain labels — no emoji = consistent alignment) -->
        <tr><td class="sect-pad" style="padding:10px 24px 0;">
          <div style="font-size:8px;color:#94A3B8;text-transform:uppercase;letter-spacing:1.2px;font-weight:800;margin-bottom:4px;">Session Details</div>
          <table width="100%" cellpadding="0" cellspacing="0" style="font-size:11.5px;color:#0F172A;">
            <tr>
              <td class="row-td" style="padding:4px 0;color:#64748B;font-weight:600;width:38%;">Charger</td>
              <td class="row-td" style="text-align:right;font-family:'SF Mono',Consolas,monospace;font-weight:700;color:#0F172A;padding:4px 0;">{charger_id}</td>
            </tr>
            <tr>
              <td class="row-td" style="padding:4px 0;color:#64748B;font-weight:600;border-top:1px solid #F1F5F9;">Connector</td>
              <td class="row-td" style="text-align:right;font-weight:700;border-top:1px solid #F1F5F9;padding:4px 0;">{connector_id}</td>
            </tr>
            <tr>
              <td class="row-td" style="padding:4px 0;color:#64748B;font-weight:600;border-top:1px solid #F1F5F9;">Started</td>
              <td class="row-td" style="text-align:right;color:#0F172A;border-top:1px solid #F1F5F9;padding:4px 0;">{started_at_str}</td>
            </tr>
            <tr>
              <td class="row-td" style="padding:4px 0;color:#64748B;font-weight:600;border-top:1px solid #F1F5F9;">Stopped</td>
              <td class="row-td" style="text-align:right;color:#0F172A;border-top:1px solid #F1F5F9;padding:4px 0;">{stopped_at_str}</td>
            </tr>
            <tr>
              <td class="row-td" style="padding:4px 0;color:#64748B;font-weight:600;border-top:1px solid #F1F5F9;">Stop reason</td>
              <td class="row-td" style="text-align:right;color:#94A3B8;font-size:11px;border-top:1px solid #F1F5F9;padding:4px 0;">{stop_reason or "—"}</td>
            </tr>
          </table>
        </td></tr>

        <!-- Payment breakdown -->
        <tr><td class="pay-pad" style="padding:10px 24px 2px;">
          <div style="font-size:8px;color:#94A3B8;text-transform:uppercase;letter-spacing:1.2px;font-weight:800;margin-bottom:4px;">Payment</div>
          <table width="100%" cellpadding="0" cellspacing="0" style="background:#F8FAFC;border-radius:8px;">
            <tr><td style="padding:8px 12px 2px;">
        ...
        <!-- Footer -->
        <tr><td class="foot-pad" style="padding:10px 24px 14px;text-align:center;">
          <div style="font-size:9px;color:#64748B;line-height:1.5;">
            Auto-generated receipt · Reply for help · <a href="https://charger.czeros.tech" style="color:#00A852;text-decoration:none;font-weight:600;">charger.czeros.tech</a>
          </div>
          <div style="margin-top:6px;padding-top:6px;border-top:1px solid #E2E8F0;font-size:8px;color:#94A3B8;letter-spacing:0.4px;">
